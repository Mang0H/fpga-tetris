...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI
	//write val via SPI
	BYTE sendBuf[2] = {reg + 2, val};
	BYTE recvBuf[2] = {0, 0};

	//read return code from SPI peripheral (see Xilinx examples) 
	//if return code != 0 print an error
	int status_code = XSpi_Transfer(&SpiInstance, sendBuf, recvBuf, 2);
	if (status_code != 0)
	{
		xil_printf ("LINE 83: ISSUE WITH THE XSpi_Transfer!!!!!");
		return;
	};
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//if return code != 0 print an error
	int status_code = XSpi_Transfer(&SpiInstance, sendBuf, recvBuf, nbytes+1);
	if (status_code != 0)
	{
		xil_printf ("LINE 115: ISSUE WITH THE XSpi_Transfer!!!!!");
		return NULL;
	};
...
BYTE MAXreg_rd(BYTE reg) {
	...
	if (status_code != 0)
	{
		xil_printf ("LINE 143: ISSUE WITH THE XSpi_Transfer!!!!!");
		return;
	};
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (status_code != 0)
	{
		xil_printf ("LINE 179: ISSUE WITH THE XSpi_Transfer!!!!!");
		return NULL;
	};
...
void MAX3421E_init(void) {
	...
	//start USB timer
	Status = XTmrCtr_Initialize(&Usb_timer, XPAR_AXI_USB_TIMER_DEVICE_ID);
	if (Status != XST_SUCCESS) {
